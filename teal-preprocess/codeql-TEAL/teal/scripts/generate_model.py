...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generates a data extensions model from a TEAL CodeQL database"
    )
    parser.add_argument("database_path", help="filepath to a TEAL CodeQL database")
    parser.add_argument(
        "-o",
        "--output",
        required=False,
...
def check_database_exists(database_path):
    if not database_path.exists():
        ...
        die("directory: " + str(database_path) + " doesn't look like a TEAL database")
...
def serialize_type(tuples, extensible_type):
    return {
        "addsTo": {"pack": "codeql/teal-all", "extensible": extensible_type},
        "data": tuples,
    }
