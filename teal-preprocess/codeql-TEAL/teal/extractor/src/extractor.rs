...
pub fn run(options: Options) -> std::io::Result<()> {
    extractor::set_tracing_level("teal");
    tracing::info!("Extraction started");
    let diagnostics = diagnostics::DiagnosticLoggers::new("teal");
    ...
    let trap_compression =
        match trap::Compression::from_env("CODEQL_EXTRACTOR_TEAL_OPTION_TRAP_COMPRESSION") {
            Ok(x) => x,
            Err(e) => {
                main_thread_logger.write(
                    main_thread_logger
    ...
    let language: Language = tree_sitter_teal::LANGUAGE.into();
    ...
    let schema = node_types::read_node_types_str("teal", tree_sitter_teal::NODE_TYPES)?;
    let erb_schema =
...
fn test_scan_coding_comment() {
    ...
    let text = "#! /usr/bin/env teal\n # encoding: utf-8";
    let result = scan_coding_comment(&text.as_bytes());
    assert_eq!(result, Some("utf-8".into()));

    let text = "\u{FEFF}#! /usr/bin/env teal\n # encoding: utf-8";
    let result = scan_coding_comment(&text.as_bytes());
    assert_eq!(result, Some("utf-8".into()));

    // A #! must be the first thing on a line, otherwise it's a normal comment
    let text = " #! /usr/bin/env teal encoding = utf-8";
    let result = scan_coding_comment(&text.as_bytes());
    assert_eq!(result, Some("utf-8".into()));
    let text = " #! /usr/bin/env teal \n # encoding = utf-8";
    let result = scan_coding_comment(&text.as_bytes());
